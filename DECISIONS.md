...
5. **A pair's capped panel said nothing.** A single map appends the cap note
   to its caption and `map_panels()` to the shared one, but a pair's panels
...
**The visual tests run on CI**, per the upgrade path recorded under "Visual
regression": one pinned image, its own baselines. The mechanism is vdiffr's
`variant` -- `_snaps/visual/` stays the local (macOS) set, and
`_snaps/linux/visual/` belongs to `.github/workflows/visual-tests.yaml`,
which pins everything typography depends on: the container tag
(`rocker/r-ver:4.6.1`), the fonts (`fonts-dejavu-core`, the only fonts on
that image), and vdiffr/svglite at the versions the macOS baselines used.
`test-visual.R` selects the variant from `FANCYMAPS_VISUAL_VARIANT`, and on a
CI machine that does not set it -- R CMD check on an arbitrary runner --
still skips, for the original reason: against the wrong platform's fonts a
diff reports typography, not regressions.

Baselines are regenerated by dispatching the workflow with
`regenerate=true`, downloading the artifact and committing it; a compare run
that finds no committed baselines fails rather than silently recording,
because a green check that recorded its own expectations checked nothing.
Bumping the container tag means regenerating in the same commit.

**The version is 0.1.0**, with a NEWS.md. Nothing about the API changed; the
point is that a downstream can now pin a release rather than a commit hash,
which is what `dsmfit`'s `versions.yml` has had to do. CRAN remains an open
question that is really a question about `fancyfx`, which is imported and
GitHub-only.

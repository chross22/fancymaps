...
needs aligning because nothing moved. The static
...
remain the answer when both have to be visible at once.
...
re-deciding the scale at the call site, and
...
styling come from one place and change in one place.
...
and `plotExtrapolation()` are statements about a *model*, not about a
map, so they belong beside the ROC curves — and what they produce is a
common thing to hand to
...
|  |  |
|----|----|
| [Choosing a scale](https://camilleross.org/fancymaps/vignettes/scales.Rmd) | why `trans = "sqrt"` is not a scale, and what replaces it |
